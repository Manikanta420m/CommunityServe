...
const getAnalytics = async (req, res) => {
  try {
    const [totals, statusBreakdown, categoryBreakdown, priorityBreakdown, recentTrend, topIssues] =
      await Promise.all([
        Issue.aggregate([
          {
            $group: {
              ...
              open: {
                $sum: { $cond: [{ $in: ["$status", ["Pending", "Under Review", "In Progress"]] }, 1, 0] },
              },
